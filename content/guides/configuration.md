...
[](pkg-opt-license) option are expected to use
...
are expected to use a licensed defined in the
[SPDX License List][spdx-licenses]. If not, a warning is generated by
default. A project can define their own custom license by adding them into
a project's `extra_licenses` option to avoid this warning:
